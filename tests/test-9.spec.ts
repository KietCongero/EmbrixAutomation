...
import accountTemplate from '../test-data/tc09-account.json';
...
test('create account, create order, manually provision, and activate service', async ({ request, page }) => {
    ...
    if (!apiUrl || !apiToken) {
        throw new Error('Missing EMBRIX_API_URL or EMBRIX_API_TOKEN');
    }
    ...
    if (activeRows.length > 0) {
        ...
        throw new Error(
            `Provisioning ID ${provisioningId} is still active. ` +
            `Cancel subscription(s) with ratingProvisioningId: ${ratingIds}`
        );
    }
    ...
    // ============================================================
    // 6. ADD INTERNET PROVISIONING DATA
    // ============================================================

    console.log('Opening INTERNET provisioning bundle...');

    await page.getByRole('heading', { name: 'Service Type: INTERNET' }).click();
    ...
    await page.getByRole('cell', { name: 'Internet IPTV', exact: true }).click();
    await page.getByRole('button', { name: 'View' }).nth(2).click();

    console.log('Adding ONT provisioning information...');

    await page.getByRole('button', { name: '+Add' }).click();

    const provisioningRow = page.locator(
        '.needs-validation > #scrollableDiv > .table > tbody > tr'
    ).last();
    ...
    console.log(`PASS: INTERNET provisioning added - ${provisioningId} / ${ontModel}`);
    ...
    await expect(
        page.getByText('Create Order successfully!')
    ).toBeVisible();
    ...
    expect(
        activeFound,
        `Service for account ${accountId} never reached ACTIVE status`
    ).toBe(true);
